...
if(isset($input["s"])) $set=preg_replace("/[^A-Za-z0-9_-]/","",$input["s"]);
else $set="Ja";
...
function getOnelangSet($langLabel)
{
	global $map;
	$s="[";
	$first=true;
	foreach($map[$langLabel] as $b)
	{
		$c=getCharList($b);
		if($c)
		{
...
if($set=="all")
{
	...
	$r="[";
	$langLabel="Ja";
	$s=getOnelangSet($langLabel);
	$r.="{\"lang\":\"".$langLabel."\",\"r\":".$s."}";
	$langLabel="Ko";
	$s=getOnelangSet($langLabel);
	$r.=",{\"lang\":\"".$langLabel."\",\"r\":".$s."}";
	$langLabel="ZhHans";
...
{
	// $set is not a lang label, it is a set label
	$s="[{";
	$s.="\"title\":\"".makeTitle($set)."\",\"chars\":\"";
	$s.=getCharList($set);
	$s.="\"}]";
	echo $s;
}
